...
public class NumberExtractor extends BaseNumberExtractor {
    ...
    private NumberExtractor(NumberMode mode, NumberOptions options) {
        ...
        this.negativeNumberTermsRegex = Pattern.compile(SwedishNumeric.NegativeNumberTermsRegex + '$', Pattern.CASE_INSENSITIVE | Pattern.UNICODE_CHARACTER_CLASS);
        ...
        switch (mode) {
            case PureNumber:
                cardinalExtractor = CardinalExtractor.getInstance(SwedishNumeric.PlaceHolderPureNumber);
                break;
            case Currency:
        ...
        if (mode != NumberMode.Unit) {
            for (Map.Entry<String, String> pair : SwedishNumeric.AmbiguityFiltersDict.entrySet()) {
