...
public class SwedishTimeParser extends BaseTimeParser {

    public SwedishTimeParser(ITimeParserConfiguration config) {
        super(config);
    }
    ...
    private DateTimeResolutionResult parseIsh(String text, LocalDateTime referenceTime) {
        ...
        ConditionalMatch match = RegexExtension.matchExact(SwedishTimeExtractorConfiguration.IshRegex, text, true);
        if (match.getSuccess()) {
