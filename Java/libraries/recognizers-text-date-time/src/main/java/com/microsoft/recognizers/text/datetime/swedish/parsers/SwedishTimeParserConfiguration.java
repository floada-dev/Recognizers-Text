...
public class SwedishTimeParserConfiguration extends BaseOptionsConfiguration implements ITimeParserConfiguration {
    ...
    private final Pattern timeSuffixFull = RegExpUtility.getSafeRegExp(SwedishDateTime.TimeSuffixFull);
    private final Pattern lunchRegex = RegExpUtility.getSafeRegExp(SwedishDateTime.LunchRegex);
    private final Pattern nightRegex = RegExpUtility.getSafeRegExp(SwedishDateTime.NightRegex);
    ...
    public SwedishTimeParserConfiguration(ICommonDateTimeParserConfiguration config) {
        ...
        atRegex = SwedishTimeExtractorConfiguration.AtRegex;
        timeRegexes = SwedishTimeExtractorConfiguration.TimeRegexList;
    }

    @Override
    public String getTimeTokenPrefix() {
        return SwedishDateTime.TimeTokenPrefix;
    }
    ...
    public PrefixAdjustResult adjustByPrefix(String prefix, int hour, int min, boolean hasMin) {
        ...
        if (trimmedPrefix.startsWith("half")) {
            ...
            Optional<Match> match = Arrays.stream(RegExpUtility.getMatches(SwedishTimeExtractorConfiguration.LessThanOneHour, trimmedPrefix)).findFirst();
